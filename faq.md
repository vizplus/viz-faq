...
Все операции в «сыром» виде, без фильтрации, представлены в сервисе [VIZ-API](https://api.dpos.space/viz/api/account_history/get_account_history). Задайте имя аккаунта, номер по порядку последней интересующей вас транзакции от новых к старым (самая последняя транзакция имеет номер минус один) и количество транзакций для вывода на экран.
