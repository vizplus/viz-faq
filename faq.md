...
- Как использовать энергию в ВИЗе?
- Как восстанавливается энергия?
- Как определяется минимально возможный размер награды?
...
Второй способ наградить кого-то - сделать это напрямую, по имени аккаунта-получателя. Для этого в Личном Кабинете [my.viz.plus](https://my.viz.plus) найдите страницу для награждения, впишите в соответствующее поле имя награждаемого аккаунта и выберите процент используемой энергии. При желании, можно добавить комментарий получателю - за что вы его награждаете.
...
Энергия в ВИЗе ограничивает использование социального капитала. Как вы знаете, при наградах сам капитал не тратится, поэтому кто-то может решить, что награждать можно бесконечно много и очень часто. Это не так: чтобы наградить кого-то, надо потратить *энергию*, имеющуюся у вашего аккаунта. Когда вся энергия потратится, аккаунт больше не сможет награждать, а также выполнять другие действия в блокчейне (они тоже требуют затрат энергии). Энергия со временем восстанавливается (см. дальше), поэтому ничего страшного не произойдёт, если она упадёт до нуля: надо просто немного подождать.

Чем больше энергии вы потратите при награждении, тем больше получится размер награды. Процент использования энергии при награде обычно можно задать в приложении, которым вы пользуетесь, но в некоторых случаях приложение само определяет, каков должен быть процент. Поэтому лучше заранее изучить этот вопрос в приложении, чтобы не потратить слишком много энергии. При награждении через Личный кабинет [my.viz.plus](https://my.viz.plus), вы сами выбираете процент награды.

Нет необходимости постоянно держать энергию аккаунта на 100%. Для размера награды важен не общий запас энергии, а процент, который вы потратите при награждении. Если вы награждаете на 5%, то награда будет совершенно одинаковой и при 10% запаса, и при 45%, и при 100%. Активные участники ВИЗа часто поддерживают запас энергии на уровне 10-20%, тратя по 1-2% энергии на каждую награду.

### Как восстанавливается энергия?

Энергия плавно восстанавливается на 20 процентных пунктов в сутки. То есть, если у вашего аккаунта осталось 28% энергии и вы больше его не трогаете, то через час энергия поднимется до 28,83%, через 12 часов - до 38%, через сутки - до 48%, через двое суток - до 68% и т.д. Но выше 100% энергия подняться не может.

### Как определяется минимально возможный размер награды?

Вы уже знаете, что размер награды в ВИЗе зависит от капитала награждающего аккаунта и потраченной на награду энергии. Иногда получается так, что участник ВИЗа наградил кого-то, всё сделав правильно, но награда не пришла. Дело в том, что в ВИЗе есть установленные делегатами ограничения на минимальный размер награждающего капитала. Они нужны, чтобы защитить блокчейн от спама огромным количеством микроскопических наград: такой поток бессмысленных награждений может затруднить работу обычных пользователей и замусорить ноды блокчейна.

На текущий момент минимальные параметры награды составляют: *капитал 5 viz при энергии 100%*. Легко посчитать, что при капитале 100 viz необходимо использовать минимум 5% энергии для награды, при капитале 500 viz достаточно 1%, а при капитале 1000 viz - 0,5%. Если тратить меньше, то энергия будет использована впустую.

Со временем делегаты могут изменить размер минимального награждающего капитала, например, при значительном увеличении ценности токена viz.
...
При голосовании сила вашего голоса, равная вашему социальному капиталу, поровну распределяется между всеми делегатами, за которых вы проголосовали. Поэтому стоит заранее решить: отдать все ваши голоса за одного делегата, чтобы помочь ему повыше подняться в списке, или разделить их между несколькими.
